...
namespace Redis.OM
{
    ...
    public class RedisConnectionProvider : IRedisConnectionProvider
    {
        ...
        public Task<AggregationResult<T>[]> AggregateAsync<T>(string indexName, string queryText = "*")
            where T : notnull => AggregateAsync<T>(CreateAggregation(indexName, queryText));
        ...
        public virtual Task<AggregationResult<T>[]> AggregateAsync<T>(RedisAggregation aggregation)
            where T : notnull => Connection.AggregateAsync<T>(aggregation);
        ...
        /// <summary>
        /// Builds the shared <see cref="RedisAggregation"/> instance used by provider-level aggregation entry points.
        /// </summary>
        /// <param name="indexName">The RediSearch index name.</param>
        /// <param name="queryText">The RediSearch query string.</param>
        /// <returns>The shared aggregation object.</returns>
        internal virtual RedisAggregation CreateAggregation(string indexName, string queryText = "*") =>
            new (indexName) { RawQuery = queryText ?? "*" };
